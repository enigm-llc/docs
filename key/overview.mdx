...
Enigm Command provides device administration, lifecycle visibility, and revocation workflows after the key is associated with the account. Initial linking remains an Enigm App workflow.
...
Emergency alert delivery and location sharing depend on device state, mobile connectivity, service availability, account association, selected contacts, and user configuration. Enigm Key should be treated as a safety-support workflow, not as a guarantee of emergency response, rescue, delivery, or availability.
...
Enigm Key is privacy-oriented emergency hardware, not a continuous tracking device.
...
Public documentation describes this authentication model at a high level suitable for external review. HMAC secrets, request formats, headers, signature placement, API routes, replay windows, internal validation logic, and operational procedures remain controlled implementation details.
...
Enigm Command is not the initial linking surface or emergency contact configuration surface. Initial linking and emergency contact configuration are controlled from Enigm App.
