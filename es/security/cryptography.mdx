...
La criptografía se utiliza en Enigm App, Enigm OS, seguridad OTA, Device Trust, comunicaciones seguras y verificación de versiones.

Los controles criptográficos respaldan:
...
flowchart TD
  ...
  Trust["Establecimiento de confianza"]
...
- Integridad de mensajes, versiones, metadatos y artefactos relevantes para la seguridad.
- Autenticidad de dispositivos, versións y partes de confianza.
- Device Trust a través de material de clave protegido y flujos de trabajo de establecimiento de confianza.
- Entrega segura de software mediante firma y verificación de versiones.
- Verificación de usuarios, dispositivos y estado de versión.
...
- Mínima exposición del material de clave.
...
- Separación entre administración y acceso a texto en claro.
...
El modelo de mensajería segura está diseñado para que el texto en claro de los mensajes esté disponible solo para endpoints de confianza autorizados. El almacenamiento en servidor, cuando es necesario para la entrega, almacena datos de mensajes cifrados en lugar de contenido de texto en claro de mensajes.

Los sistemas administrativos no proporcionan acceso de texto en claro a los mensajes.
...
A nivel de arquitectura pública, la mensajería segura de Enigm combina cifrado de contenido simétrico, objetivos de establecimiento de claves poscuánticas, objetivos de firma poscuánticas, material de claves protegido, confianza vinculada al dispositivo y flujos de trabajo de verificación.

La confidencialidad de los mensajes depende de los dispositivos endpoints autorizados y del material de claves protegido. Los sistemas en servidor, los flujos de trabajo Enigm Command, los administradores Enigm Server, los sistemas de monitorización y los sistemas operativos no deben convertirse en rutas de acceso de texto en claro.
...
- Almacenamiento protegido de material de claves gestionado.
...
- Reducción de la exposición del material de clave al tiempo de ejecución de la aplicación y a los flujos de trabajo administrativos.
...
La administración de claves respaldada por HSM protege las claves de plataforma administradas y las operaciones criptográficas sensibles a la seguridad. No debe describirse como un mecanismo que otorga a los administradores de Enigm acceso al texto en claro de mensajes, texto en claro de archivos adjuntos, comunicaciones de usuarios o material de clave privada en poder de dispositivos de usuarios de confianza.
...
La criptografía poscuántica se utiliza como parte de una arquitectura más amplia. No reemplaza Device Trust, el almacenamiento de claves protegido, el cifrado de extremo a extremo, los flujos de trabajo de verificación, la gobernanza de la seguridad ni el refuerzo de endpoints.
...
## Establecimiento de claves poscuánticas
...
- Preservar la separación entre el material de claves mantenido en el punto final y los sistemas de entrega en servidor.
...
## Modelo de distribución de secretos
...
El modelo tiene como objetivo garantizar que el material criptográfico protegido o el contexto de autorización no queden expuestos a través de un sistema en servidor, un administrador, una capa de almacenamiento o un flujo de trabajo operativo.
...
- Resistencia al acceso administrativo unilateral a texto en claro.
...
La distribución secreta no otorga a los administradores de Enigm acceso al texto en claro de los mensajes. Es un control de defensa en profundidad que complementa el material de claves protegido en el punto final, el cifrado de extremo a extremo y Device Trust.
...
## Línea base TLS de suites de cifrado
...
La documentación del conjunto de cifrado se limita a la postura del transporte web público. No debe interpretarse como una divulgación de la topología del servicio interno, los endpoints privados, los aspectos internos del protocolo de aplicación o los detalles del protocolo de cifrado de mensajes.
...
El material de clave privada está diseñado para permanecer asociado con dispositivos de confianza.
...
Cuando los límites del hardware no puedan almacenar directamente material criptográfico de mayor tamaño, se podrá utilizar un envoltorio protegido o material de control de acceso para proteger el material de clave fuera de los límites seguros del hardware.

El material de clave privada no debe almacenarse en texto en claro.

El almacenamiento seguro reduce la exposición de las claves, pero no elimina el riesgo de comprometer los endpoints.
...
- Controles de autenticidad de versión.
- Verificación del manifiesto.
- Verificación de artefactos.
- Revisión del estado de confianza.

La verificación tiene como objetivo reducir la dependencia de la confianza implícita. No reemplaza la concienciación del usuario, la seguridad del dispositivo ni el material de clave protegido.
...
La criptografía OTA no reemplaza el arranque verificado, Remote Attestation, los controles de producción, la evaluación Device Trust ni la verificación del cliente.
...
- Requisitos de firma de versión.
...
- Usuarios autorizados que divulgan texto en claro.
