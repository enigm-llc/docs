...
- La visibilidad administrativa se mantiene separada del acceso al texto en claro.
...
El registro de cuenta estándar de Enigm utiliza un modelo que minimiza la identidad y no requiere un correo electrónico, un número de teléfono ni un documento de identidad. La creación de cuentas estándar utiliza autenticación de nombre de usuario y contraseña, generación y manejo de frases de recuperación, creación de identidad local del dispositivo y asociación explícita de dispositivos de confianza.
...
Enigm incluye múltiples capas diseñadas para reducir la exposición de los metadatos y la visibilidad de los patrones de comunicación.
...
La selección de la región del servidor es un control de implementación y producto para entornos Enigm Server. No crea un modelo de acceso a texto en claro diferente y no debilita el cifrado de extremo a extremo de Enigm, Device Trust, ni la minimización de metadatos.
...
Las sesiones de Enigm App están limitadas a 6 horas. El estado de la sesión se trata como un estado de acceso a la cuenta y permanece separado de Device Trust, el estado de recuperación, el material de clave protegido y el texto en claro del mensaje.

El manejo de Enigm App seguro de medios está diseñado para reducir la exposición innecesaria de texto en claro para archivos, imágenes, videos, archivos adjuntos y otros medios multimedia compatibles. Las políticas de conversación y de grupo pueden controlar el envío, el reenvío, la eliminación y el manejo de medios sin dar a los sistemas administrativos acceso en texto en claro al contenido protegido.

Los controles de resistencia a la captura reducen la captura de pantalla, la grabación de pantalla, la vista previa o la exposición a exportaciones externas según la capacidad y la política del dispositivo. Estos controles complementan el cifrado de extremo a extremo y Device Trust; no garantizan la protección contra endpoints comprometidos, cámaras externas, usuarios autorizados maliciosos o entornos operativos modificados.
...
Enigm eSIM utiliza un modelo de compra y gestión del ciclo de vida por parte de Enigm que minimiza la identidad para la conectividad móvil solo de datos. El flujo de trabajo por parte de Enigm no recopila verificación KYC, correo electrónico, número de teléfono ni documento de identidad.

Los metadatos del ciclo de vida de Enigm eSIM siguen limitados a la operación de conectividad, la asociación de cuentas de Enigm, los derechos, el soporte y las necesidades de seguridad. El estado de conectividad permanece separado del texto en claro del mensaje, el contenido de la llamada segura, los medios, los archivos adjuntos, las conversaciones de los usuarios y el material de clave privada.

Los registros de tráfico de la capa del operador, los registros de asignación de IP del operador, los registros de acceso de radio, los registros de enrutamiento de paquetes, los registros de conexión del operador, los registros de itinerancia del operador y los registros de uso de la red de operador están fuera de las categorías de metadatos mantenidos por Enigm cuando los genera o retiene un proveedor de infraestructura de telecomunicaciones independiente.
...
Los registros de auditoría y los metadatos de seguridad utilizan protección en capas, incluido el cifrado parcial cuando lo admite el dominio de almacenamiento relevante y Privacy-Preserving Device Handles cuando se requiere correlación de dispositivos. No contienen texto en claro de mensajes, texto en claro de archivos adjuntos, contenido de llamadas seguras, conversaciones de usuarios ni material de clave privada.
...
- Separación entre sistemas administrativos y acceso a texto en claro.
