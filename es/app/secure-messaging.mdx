...
- El texto en claro del mensaje no está diseñado para que los componentes en servidor puedan acceder a él.
- El almacenamiento de mensajes en servidor, cuando es necesario para la entrega, almacena únicamente datos de mensajes cifrados.
...
- El manejo seguro de medios está diseñado para reducir la exposición del texto en claro para archivos, imágenes, videos y otros medios multimedia compatibles.
- Los controles administrativos no deben otorgar acceso al texto en claro del mensaje.
...
La mensajería Enigm está diseñada para que el contenido del mensaje de texto en claro se prepare y se cifre dentro del contexto Enigm App en el remitente antes de la entrega.
...
4. El texto en claro del mensaje se cifra localmente en Enigm App.
5. Los datos del mensaje cifrado se envían para su entrega.
6. Enigm App en el destinatario verifica la elegibilidad del dispositivo y el estado de la clave protegida.
7. El contenido del mensaje se descifra localmente sólo cuando esté permitido.

Los componentes en servidor no deberían requerir texto en claro de mensajes para enrutar, almacenar, sincronizar o caducar mensajes.

## Ciclo de vida del mensaje

El ciclo de vida del mensaje se estructura en torno a la protección en cliente y la autorización basada en el dispositivo.
...
- Verificación en el destinatario.
...
Es posible que se requiera almacenamiento de mensajes en servidor para la entrega, sincronización, recuperación fuera de línea, coordinación de caducidad o gestión del ciclo de vida resistente al abuso.

Cuando se requiere almacenamiento en servidor, este debe almacenar únicamente datos de mensajes cifrados. El texto en claro del mensaje no está diseñado para que los componentes en servidor puedan acceder a él.

Los sistemas en servidor pueden procesar metadatos limitados necesarios para:
...
Los visores seguros deben manejar el contenido de mensajes y archivos adjuntos sin exportar texto en claro a aplicaciones externas. Cuando la acción o política del usuario permite compartir o exportar externamente, esa acción está fuera del modelo normal de visualización protegida y debe tratarse como un límite de divulgación.
...
- Almacene los datos adjuntos como material cifrado donde se requiere almacenamiento en servidor.
...
- Evite la exportación de texto en claro a aplicaciones externas durante la visualización protegida normal.
...
Los permisos de grupo están diseñados para admitir espacios de comunicación controlados y al mismo tiempo preservar el modelo de cifrado de un extremo a otro. Los controles administrativos o a nivel de grupo no deben crear acceso a texto en claro para sistemas en servidor o flujos de trabajo Enigm Command.
...
Los permisos de grupo deben poder auditarse a nivel de política y ciclo de vida sin exponer el texto en claro del mensaje, el texto en claro de los archivos adjuntos, el texto en claro de los medios, las conversaciones de los usuarios o el material de clave privada.
...
Los controles de eliminación operan sobre objetos de contenido cifrados, estado del ciclo de vida local y disponibilidad de la conversación. La eliminación no implica que los administradores puedan leer el texto en claro de los mensajes, el texto en claro de los archivos adjuntos, el texto en claro de los medios, las conversaciones de los usuarios o el material de clave privada.
...
El manejo de Enigm App seguro de medios está diseñado para reducir la exposición innecesaria de texto en claro durante el uso normal.
...
Las protecciones de captura y grabación de pantalla se aplican según la capacidad del sistema operativo y la política del dispositivo. Estas protecciones están diseñadas para reducir la captura accidental o no autorizada durante el uso normal de la aplicación, pero no pueden garantizar la prevención contra todos los métodos de captura, dispositivos comprometidos, cámaras externas o entornos operativos modificados.
...
Según la capacidad y la política del dispositivo, Enigm App aplica controles diseñados para reducir:

- Captura de pantalla.
- Grabación de pantalla.
- Vista previa de fugas.
- Exportación innecesaria de texto en claro a aplicaciones externas.
...
Los controles administrativos deben exponer evidencia de políticas y ciclo de vida sin exponer el texto en claro de los mensajes.
