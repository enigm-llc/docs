...
La seguridad OTA reduce el riesgo de entrega de software al separar la autoridad de versiones, el manejo de solicitudes, la confianza de los metadatos, la integridad de los artefactos, la elegibilidad del dispositivo y la verificación en el dispositivo.
...
La firma de producción establece un origen de versión de confianza.

Esta capa respalda:

- Autorización de liberación.
- Hardware-Backed Signing.
- Origen de versión de confianza.
...
- Autenticidad de versión de los metadatos.
- Evidencia de aprobación de metadatos de versión OTA.

Esta autoridad no es la misma que la autoridad objetivo de firma de versiones de producción.

### Autoridad objetivo: Autoridad de firma de versiones de producción

La Autoridad de firma de versións de producción es la autoridad de destino para la firma de artefactos de versión y imágenes de producción.

Esta autoridad está diseñada para ser responsable de:

- Firma de imagen de producción.
- Firma de carga útil OTA.
- Artefactos de versión críticos para la firma.
- Autorización de salida a producción.

Esta autoridad está diseñada para operar a través de un HSM físico dedicado y debe permanecer distinta de la Autoridad de firma del manifiesto OTA.
...
El modelo actual autoriza manifiestos y metadatos OTA. Proporciona autenticidad de versión para la capa de manifiesto, no autoridad de firma de imágenes de producción completa.

### Material de firma protegido

La clave de firma del manifiesto no debe almacenarse en repositorios de Git, variables CI/CD, scripts de compilación, estaciones de trabajo de desarrolladores, almacenamiento de objetos en línea ni artefactos de versión.
...
## Arquitectura HSM objetivo de firma de versiones

La arquitectura de firma de versión de producción Enigm OS de destino está diseñada para utilizar un HSM físico dedicado.
...
La autoridad de firma de versiones de producción de destino no es la misma autoridad que la autoridad de firma de manifiesto OTA actual. La autoridad de destino está diseñada para actuar como raíz de confianza de la autorización de versión para los artefactos de producción.
...
2. El pipeline de versiones prepara las cargas de firma.
...
```mermaid
sequenceDiagram
  participant Compilación as Compilación Artifacts
  participant Pipeline as Pipeline de versiones
  participant Operator as Operator Approval
  participant Authority as Autoridad de firma
  participant Verify as Verification
  participant Gates as Controles de versión
  participant Publish as Publication

  Compilación->>Pipeline: Provide versión artifacts
  ...
  Gates->>Publish: Approve versión publication
```
...
La evidencia de atestación puede incluir:
...
La evidencia de atestación se interpreta como evidencia de integridad para las decisiones de elegibilidad. No sustituye a la verificación de manifiesto, la verificación de artefactos ni la firma de versión.
...
La evidencia de atestación es sensible a la frescura.
...
El almacenamiento a largo plazo debería preferir los resultados de las políticas en lugar de las cargas de firma de certificación sin procesar.
...
- Metadatos de versión modificados.
...
Hardware-Backed Signing reduce la exposición de claves y admite la autorización de versión, pero no prueba que el software lanzado esté libre de defectos o lógica maliciosa introducida antes de la firma.
