...
The Enigm privacy model applies across Enigm App, Enigm Command, Enigm Server, Enigm OS, Trust Security Center, Enigm Proxy, VPN Service, Tor Gateway, Threat Intelligence Platform, Enyra, Enigm Key, Enigm eSIM, and OTA Architecture.
...
Standard Enigm account creation uses an identity-minimizing model and does not require an email address, phone number, or identity document. Account creation is performed through Enigm Command and uses username and password authentication, recovery phrase generation and handling, 6-digit PIN setup, account identity creation, and explicit trusted-device association.
...
Metadata reduction is intended to lower exposure and reduce confidence in simple communication-pattern inference. It should not be interpreted as an absolute identity protection, untraceability, or complete traffic-analysis resistance claim.
...
Enigm App sessions use a 1-hour validity window and are automatically renewed while the session remains eligible under account, device, and policy controls. Session state is treated as account access state and remains separate from Device Trust, recovery state, protected key material, and message plaintext.

Enigm App access is protected by a 6-digit numeric PIN after authentication. PIN validation is performed against the Enigm server-side security layer rather than as a local-only device check. The app can prompt for the PIN immediately or after a user-defined inactivity period.

When the app is closed or PIN-gated, Enigm App clears sensitive runtime state from memory and removes decrypted runtime material from the active app context. Sensitive runtime data is protected while resident in memory according to the Enigm App runtime protection model.

Reverse PIN and repeated incorrect PIN workflows can record a profile risk state that is visible to contacts. This risk state exists to help contacts understand that the user may be compromised or under pressure; it is cleared only after successful PIN validation by the user.

Contact discovery uses unique contact identifiers. Users cannot be searched by username or display name. Users can choose whether they are discoverable through the public Enigm discovery surface or only available through Enigm Server private environments.
...
Enigm Proxy and VPN Service use policy metadata to enforce access, evaluate risk, or support troubleshooting for Enigm App network privacy and traffic-separation flows. Tor Gateway uses policy metadata for supported public web access paths.
