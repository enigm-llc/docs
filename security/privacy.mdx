...
Enigm-controlled servers and services are operated under Enigm's Swiss subsidiary and Swiss legal governance. For processing activities involving users in the European Union, Enigm is designed to align with GDPR/RGPD principles including purpose limitation, data minimization, storage limitation, confidentiality, integrity, and accountability.
...
Server region selection is a product and deployment control for Enigm Server environments. It does not create a different plaintext access model and does not weaken Enigm end-to-end encryption, Device Trust, or metadata minimization.
...
VPN Service and Proxy Network use policy metadata to enforce access, evaluate risk, or support troubleshooting for Enigm App network privacy and traffic-separation flows. Tor Gateway uses policy metadata for selected public web access paths when enabled.
...
Audit records and security metadata use encrypted storage and Privacy-Preserving Device Handles where device correlation is required. They do not contain message plaintext, attachment plaintext, secure call content, user conversations, or private key material.
