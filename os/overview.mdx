...
For the user-facing first-run setup flow, see [Enigm OS Quickstart](/quickstarts/enigm-os).
...
Enigm OS is currently supported only on Fairphone 6.

This hardware scope is intentional. Enigm OS security depends on a controlled device baseline, verified software state, production update compatibility, hardware behavior, and repeatable security validation. Limiting support to a validated device model reduces unmanaged hardware variation and helps preserve the expected Device Trust model.

Devices outside the supported hardware scope should not be treated as production Enigm OS devices.
...
Enigm Proxy, VPN Service, Enigm eSIM, and Tor Gateway are separate platform components. Enigm OS may provide device-level network policy or posture signals, but network services and selected web access paths remain distinct from OS trust and app-level cryptography.
