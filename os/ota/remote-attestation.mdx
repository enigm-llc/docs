...
The Enigm OS OTA architecture is designed to support remote attestation as an additional production hardening layer. Remote attestation can be used to help determine whether a device is eligible for enrollment, registration, protected update metadata, private update artifacts, or sensitive rollout channels.

Remote attestation is an intended production hardening layer once enabled in the client integration. This document does not claim that every production device is necessarily attested on every request.

This document is intended for Android engineers, security auditors, enterprise customers, and technical partners.

## Overview

Remote attestation is designed to support eligibility decisions based on device-produced security evidence.

Within the Enigm OS OTA model, remote attestation is intended to help determine whether a device is eligible to:
...
Remote attestation may be used to evaluate:
...
The backend may require remote attestation for selected protected workflows once the client integration supports it. Attestation can be used to determine eligibility, but it should be evaluated alongside other OTA security controls.
...
Device eligibility should not rely only on attestation. Eligibility decisions should consider enrollment state, device handles, rollout policy, channel policy, and request context where supported.
...
Remote attestation should be evaluated as an intended production hardening layer within the broader Enigm OS OTA security model.
