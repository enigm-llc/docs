...
Remote attestation can provide an additional eligibility signal.
...
Request authentication should be evaluated alongside freshness validation, replay resistance, request integrity, device eligibility, and remote attestation where supported.
...
OTA should use privacy-preserving device handles where possible. Device identifiers should be scoped to update eligibility and device lifecycle needs rather than unnecessary identity collection.
