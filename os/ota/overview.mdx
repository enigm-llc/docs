...
Trust Security Center may surface update-related posture where supported, but it does not replace OTA eligibility or release verification.

## Relationship With Remote Attestation

Remote attestation is an additional eligibility signal.

Remote attestation can complement OTA security controls by contributing device integrity evidence before a release is offered or installed where supported.
...
- Remote attestation is a signal, not a complete guarantee of device safety.
- Update installation does not guarantee that all future device behavior remains trusted.
