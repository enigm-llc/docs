...
Enigm OS updates are delivered through the Enigm OS Updater on eligible devices. The public OTA surface is informational and does not provide manual download links for protected update artifacts.
...
- Rollout policy.
- Client verification.
- Privileged updater execution on Enigm OS.

The Enigm OS Updater is part of the trusted device software environment. It is not treated as a normal third-party application for update installation decisions.
...
Protected OTA enrollment is a controlled workflow. Devices are expected to establish authorized eligibility before accessing protected update metadata or private update artifacts. Enigm OS does not treat open, unauthenticated self-enrollment as sufficient for protected OTA access.

Enrollment is preauthorized. A device must present authorized device identity evidence and a valid enrollment request before it can receive protected OTA credentials or access private OTA workflows. Renewal for an already enrolled device is bound to the existing device identity rather than treated as a new open registration.

Eligibility data should remain privacy-minimized. OTA workflows should use Privacy-Preserving Device Handles, cryptographic request authentication, release channel state, and device compatibility information rather than unnecessary personal identifiers.
...
Protected update packages are intended to be installed by Enigm OS Updater. Enigm does not document a public manual download workflow for production OTA packages because update trust depends on eligibility, signed metadata, artifact verification, rollback policy, and privileged client validation.
...
- Manifest expiration.
- Device and release-channel match.
- Anti-rollback policy.
- Security patch progression.
...
The client enforces rollback constraints defined by release and device policy. The client rejects releases that violate rollback policy.
...
Remote Attestation is an additional eligibility layer, not a substitute for signed manifests, signed release artifacts, client verification, or controlled rollout policy.
