...
In this documentation, "messages" refers to the protected communication content handled by Enigm messaging workflows. This includes text messages, voice messages, call records, encrypted files, images, videos, multimedia, attachments, and related protected conversation objects unless a section narrows the term.
...
This page describes the messaging security model. The broader Enigm cryptographic architecture, including post-quantum cryptography, key lifecycle, secure storage, device-bound trust, and verification workflows, is documented in [Cryptography](/security/cryptography).
...
- Sensitive runtime state is cleared when Enigm App is closed or PIN-gated.
- Sensitive runtime data is protected while resident in memory according to the Enigm App runtime protection model.
...
PIN-gated app access is separate from message encryption. PIN validation is performed against the Enigm server-side security layer and does not provide administrative access to message plaintext or private key material.
...
In direct user-to-user conversations, deletion can be scoped to the requesting user only or applied for all participants where policy allows. "Delete for me" removes the content from the requesting user's available conversation state. "Delete for everyone" removes the protected content from the conversation availability state for both participants according to policy.

In group conversations, group administrators can delete protected content according to group policy. Group policy can also allow users to delete their own messages. In this context, messages include text messages, voice messages, call records, encrypted files, images, videos, multimedia, attachments, and related protected conversation objects.
...
Cadence, timing characteristics, generation logic, runtime tuning details, and implementation-sensitive behavior remain controlled implementation details. The public model documents the privacy objective and architectural boundary of traffic-shaping controls.
