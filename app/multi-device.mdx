---
title: Multi-Device
description: Public multi-device trust architecture for Enigm App.
---
...
Enrollment should verify that the new device is authorized to join the account context. Existing trusted devices may participate in enrollment workflows where supported. Managed deployments may also use Enigm Command policy to review or approve enrollment.
...
Device association should use privacy-preserving identifiers where possible. These identifiers support lifecycle review, policy evaluation, and audit correlation without exposing unnecessary public identity metadata.
...
Revocation does not guarantee removal of content already received and decrypted on a device before revocation. This is a security limitation of any model where an endpoint previously had authorized access.
...
- Existing trusted devices may participate in enrollment where supported.
...
flowchart TD
  ...
  A -. "trust establishment where supported" .-> Candidate
  Control -. "managed review where applicable" .-> Candidate
...
- Revocation affects future trust decisions but cannot guarantee removal of content already accessed by the revoked device.
