...
Active Defense is separate from Enigm App runtime protection. Runtime protection hardens the application binary and runtime against tampering, debugging, instrumentation, root or jailbreak risk, and reverse-engineering workflows. Active Defense evaluates security-relevant network behavior and malware-risk context during bounded assessment windows.
...
Detection logic, rules, thresholds, signatures, tooling, and operational parameters remain controlled security material. Public documentation describes the signal categories required to evaluate the privacy and security model.
...
Detection rules, signatures, thresholds, intelligence sources, correlation logic, and operational parameters remain controlled security material. The categories above describe the public analytical model without turning detection behavior into an evasion guide.
...
Assessment duration and report handling are product configuration concerns. The public model documents user visibility, privacy boundaries, and retention behavior without publishing timing values, capture parameters, analysis thresholds, or operational workflow details.
...
Enigm Intelligence may correlate Active Defense findings with other security signals to support investigation, risk assessment, defensive response, and authorized user visibility. Correlation must preserve access controls, data minimization, and content confidentiality.
