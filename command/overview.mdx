...
Enigm Command supports Tor Gateway access for selected public or administrative web surfaces where the access path is enabled.

Tor Gateway access is intended to support privacy-oriented access paths for:

- Public web surfaces.
- Enigm Command access paths where enabled.
- Documentation-safe administrative web surfaces.

Tor Gateway access is not Enigm Server, not a messaging client, and not a general-purpose infrastructure access path. It must not be used for internal APIs, development systems, operational tooling, sensitive infrastructure management, or non-public service exposure.
...
Enigm Key initial linking and emergency contact configuration are controlled from Enigm App. Enigm Command should not be documented as the initial linking surface or emergency contact configuration surface.
