...
Enigm Command uses authorized Enigm account context for access to account and administrative workflows. Authentication internals, session implementation, token formats, route structure, and operational access-control details remain controlled implementation details.
...
Enigm Command governs Tor Gateway lifecycle and policy for supported public web surfaces.
...
If a request moves into security investigation, threat intelligence access, risk analysis, or defensive operations support, it belongs to Enyra security intelligence workflows documented under Intelligence.
